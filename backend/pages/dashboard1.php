...
<body class="d-flex flex-column h-100">
  ...
  <script src="/web/assets/vendor/Chart.js/Chart.min.js"></script>
  <script>
    $(document).ready(function() {
      // ----------------- Tổng số mặt hàng --------------------------
      function getDuLieuBaoCaoTongSoMatHang() {
        $.ajax('/web/backend/api/baocao_tongsomathang.php', {
      ...
      function getDuLieuBaoCaoTongSoKhachHang() {
        $.ajax('/web/backend/api/baocao_tongsokhachhang.php', {
          success: function(data) {
            var dataObj = JSON.parse(data);
            var htmlString = `<h1>${dataObj.SoLuong}</h1>`;
      ...
      function getDuLieuBaoCaoTongSoDonHang() {
        $.ajax('/web/backend/api/baocao_tongsodonhang.php', {
          success: function(data) {
            var dataObj = JSON.parse(data);
            var htmlString = `<h1>${dataObj.SoLuong}</h1>`;
      ...
      function getDuLieuBaoCaoTongSoGopY() {
        $.ajax('/web/backend/api/baocao_tongsogopy.php', {
          success: function(data) {
            var dataObj = JSON.parse(data);
            var htmlString = `<h1>${dataObj.SoLuong}</h1>`;
      ...
      function renderChartThongKeLoaiSanPham() {
        $.ajax({
          url: '/web/backend/api/baocao_thongkeloaisanpham.php',
          type: "GET",
